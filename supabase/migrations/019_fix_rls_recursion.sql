...
-- Problème : "infinite recursion detected in policy for relation super_admins"

-- 1. Désactiver temporairement RLS pour corriger les politiques
ALTER TABLE public.super_admins DISABLE ROW LEVEL SECURITY;

-- 2. Supprimer toutes les politiques existantes qui causent la récursion
...
-- 4. Créer des politiques RLS simples sans récursion
-- Politique pour permettre l'insertion initiale (pas de récursion)
CREATE POLICY "super_admins_insert_initial" ON public.super_admins
    FOR INSERT WITH CHECK (true);

-- Politique pour permettre la lecture (vérification simple)
CREATE POLICY "super_admins_select_simple" ON public.super_admins
    FOR SELECT USING (true);

-- Politique pour permettre la mise à jour (vérification simple)
CREATE POLICY "super_admins_update_simple" ON public.super_admins
    FOR UPDATE USING (true);

-- Politique pour permettre la suppression (vérification simple)
...
-- 8. Fonction is_super_admin simplifiée (sans récursion)
CREATE OR REPLACE FUNCTION is_super_admin(user_id UUID DEFAULT auth.uid())
RETURNS BOOLEAN AS $$
BEGIN
    -- Vérification simple sans récursion
    RETURN EXISTS (
...
-- 9. Fonction create_super_admin simplifiée
...
BEGIN
    ...
    -- Vérifier si un Super-Admin existe déjà (vérification simple)
...
-- 12. Commentaire
COMMENT ON TABLE public.super_admins IS 'Table des Super-Administrateurs de l''application (politiques RLS simplifiées)';
