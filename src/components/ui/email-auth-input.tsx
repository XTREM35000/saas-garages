import React, { useEffect, useId, useMemo, useState, useCallback, memo } from 'react'
...
export const EmailAuthInput: React.FC<EmailAuthInputProps> = memo(({
  label = 'Email',
  value,
  onChange,
  slug,
  ...
  useEffect(() => {
    ...
    if (slug) {
      ...
      if (value.endsWith(slugSuffix)) {
        const newLocalPart = value.slice(0, -slugSuffix.length)
...
EmailAuthInput.displayName = 'EmailAuthInput'
